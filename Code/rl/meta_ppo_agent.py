...
class MetaPPOAgent(BaseAgent):
    ...
    def __init__(self, config, ob_space):
        ...
        # build up networks
        self._actor = MlpActor(config, ob_space, ac_space, tanh_policy=True)
        self._old_actor = MlpActor(config, ob_space, ac_space, tanh_policy=True)
        self._critic = MlpCritic(config, ob_space)
        self._network_cuda(config.device)
        print('\n actor in Metappo',self._actor)
